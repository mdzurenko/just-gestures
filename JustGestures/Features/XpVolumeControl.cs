...
namespace JustGestures.Features
{
    ...
    public class XpVolumeControl : IVolumeControl
    {

        public const int ComponentTypeDstFirst = 0x0;
        public const int ComponentTypeSrcFirst = 0x1000;
        public enum ComponentType : int
        {
            DstDigital = (ComponentTypeDstFirst + 1),
            DstHeadphones = (ComponentTypeDstFirst + 5),
            DstLast = (ComponentTypeDstFirst + 8),
            DstLine = (ComponentTypeDstFirst + 2),
            DstMonitor = (ComponentTypeDstFirst + 3),
            DstSpeakers = (ComponentTypeDstFirst + 4),
            DstTelephone = (ComponentTypeDstFirst + 6),
            DstUndefined = (ComponentTypeDstFirst + 0),
            DstVoiceIn = (ComponentTypeDstFirst + 8),
            DstWaveIn = (ComponentTypeDstFirst + 7),
            SrcAnalog = (ComponentTypeSrcFirst + 10),
            SrcAuxiliary = (ComponentTypeSrcFirst + 9),
            SrcCompactDisc = (ComponentTypeSrcFirst + 5),
            SrcDigital = (ComponentTypeSrcFirst + 1),
            SrcFirst = 0x1000,
            SrcLast = (ComponentTypeSrcFirst + 10),
            SrcLine = (ComponentTypeSrcFirst + 2),
            SrcMicrophone = (ComponentTypeSrcFirst + 3),
            SrcPcSpeaker = (ComponentTypeSrcFirst + 7),
            SrcSynthesizer = (ComponentTypeSrcFirst + 4),
            SrcTelephone = (ComponentTypeSrcFirst + 6),
            SrcUndefined = (ComponentTypeSrcFirst + 0),
            SrcWaveout = (ComponentTypeSrcFirst + 8)
        }
        ...
        public float MasterVolume
        {
            get
            {
                ...
                mixerClose(mixer);
                float volume = currentVol / (float)volCtrl.lMaximum;
                return volume;
            }
            set
            {
                ...
                int type = MIXERCONTROL_CONTROLTYPE_VOLUME;
                GetVolumeControl(mixer, MIXERLINE_COMPONENTTYPE_DST_SPEAKERS, type, out volCtrl, out currentVol);
                volume *= volCtrl.lMaximum;
                SetVolumeControl(mixer, volCtrl, (int)volume);
                mixerClose(mixer);
            }
        }
        ...
        public bool Mute
        {
            get
            {
                ...
                GetVolumeControl(mixer, MIXERLINE_COMPONENTTYPE_DST_SPEAKERS, type, out volCtrl, out currentVol);
                mixerClose(mixer);
                return currentVol == 0 ? false : true;
            }
            set
            {
                int mixer;
                ...
                int type = MIXERCONTROL_CONTROLTYPE_MUTE;
                GetVolumeControl(mixer, MIXERLINE_COMPONENTTYPE_DST_SPEAKERS, type, out volCtrl, out currentVol);
                int intMute = value == true ? 1 : 0;
                SetVolumeControl(mixer, volCtrl, intMute);
                mixerClose(mixer);
            }
        }
        ...
        {
            float volume = this.MasterVolume;
            this.MasterVolume = volume < 0.9F ? volume + 0.1F : 1F;
        }

        public void VolumeDown()
        {
            float volume = this.MasterVolume;
            this.MasterVolume = volume > 0.1F ? volume - 0.1F : 0F;
        }
    }
}
